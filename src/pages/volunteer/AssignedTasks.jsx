...
import { Filter, Eye, AlertCircle, RefreshCw } from "lucide-react";
...
export default function AssignedTasks() {
  ...
  useEffect(() => {
    const loadAssigned = async () => {
      ...
      try {
        ...
        // Filter issues assigned to this volunteer
        setIssues(list.filter(i => i.assignedVolunteerId === currentUser.uid));
        setCategories(cats);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  return (
    <div className="animate-fade container">
      <h2 style={{ fontSize: "2rem", marginBottom: "0.5rem", fontFamily: "var(--font-heading)" }}>Assigned Tasks</h2>
      <p style={{ color: "var(--text-secondary)", marginBottom: "2rem", fontSize: "0.95rem" }}>
        List of local civic problems you have volunteered to verify or repair.
      </p>
      ...
      <div className="glass-card" style={{ padding: "1rem", marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", gap: "1rem", alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginLeft: "auto" }}>
            Assigned Tasks: <strong>{filteredIssues.length}</strong>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card" style={{ padding: "1.5rem" }}>
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "3rem", color: "var(--text-muted)" }}>
            ...
            <p>No active tasks match the filters.</p>
          </div>
        ) : (
